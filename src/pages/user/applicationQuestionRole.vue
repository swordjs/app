<template>
  <view class="questionBox">
    <view class="form">
      <i-form>
        <i-field required label="反馈微信号/QQ号">
          <i-input placeholder="请填写" v-model="form.contactDetails"></i-input>
        </i-field>
        <i-field required label="擅长领域">
          <i-input
            placeholder="前端/后端/算法/编译原理"
            v-model="form.filed"
          ></i-input>
        </i-field>
        <i-field label="您的身份">
          <i-input
            placeholder="开发者/创业者/学生"
            v-model="form.identity"
          ></i-input>
        </i-field>
        <i-field label="社交主页地址">
          <i-input
            placeholder="Github/知乎/掘金/博客"
            v-model="form.socialHomepage"
          ></i-input>
        </i-field>
      </i-form>
        ...
        if (form.contactDetails === "" || form.filed === "") {
          ...
          if (postResult.success) {
            uni.showModal({
              title: "提示",
              content:
                "您的申请已经提交成功，请耐心等待审核。请关注剑指题解公众号或者加入交流群获取快速审核通道~",
              showCancel: false,
              confirmText: "好的",
