          ...
          hide: !show,
			...
			<view class="mainBox">
				...
				<view class="userInfo" @click="handleLogin">
					<!-- 头像 -->
					<view class="avatar">
						<image :src="!isLogin ? '../../../../static/index/user.png' : user.avatarUrl" />
					</view>
					<!-- 昵称 -->
					<view class="info">
				...
				<view class="menu">
					<view class="menuItem" @click="handleClickItem('/pages/user/setting')">
						<i-icon name="settings-2-fill" color="#FFCC5C"></i-icon>
						<text>设置</text>
					</view>
					<!-- #ifndef MP-QQ -->
					<view class="menuItem"
						@click="handleClickItem('/pages/common/webview?url=https://www.yinzhuoei.com/index.php/archives/515/')">
			...
			<view class="rowToggle" @click.stop="handleToggle">
				<i-icon @click="handleToggle" :class="{
            hide: !show,
          }" :name="`arrow-${show ? 'left' : 'right'}-s-line`" color="#FFCC5C"></i-icon>
			</view>
		</view>
		<!-- 蒙层 -->
