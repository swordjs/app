...
exports.startup = async (e, t, r) => {
  ...
  if (E) {
    ...
    if (
      ...
      (n = l[i.routeKey]),
      (c = l[i.methodKey]),
      delete l[i.serviceKey],
      delete l[i.actionKey],
      delete l[i.routeKey],
      delete l[i.methodKey],
      (p = l),
      'GET' === e.httpMethod)
    );
    else {
      var h = {},
        f = e.body;
      e.isBase64Encoded && e.body && (f = new Buffer(e.body, 'base64').toString()),
        e.headers['content-type'] &&
          (e.headers['content-type'].includes('application/json')
            ? f && (h = JSON.parse(f))
            : e.headers['content-type'].includes('application/x-www-form-urlencoded') &&
              f &&
              f.split('&').forEach((e) => {
                var t = e.split('='),
                  r = t[0],
        ...
        'string' == typeof h.params && (h.params = JSON.parse(h.params)),
        (a = h[i.serviceKey] || a),
        (s = h[i.actionKey] || s),
        (n = h[i.routeKey] || n),
        (c = h[i.methodKey] || c),
        'object' == typeof h[i.paramsKey] && (p = Object.assign(p, h[i.paramsKey]));
    }
  ...
  let g = () => {
    ...
    u = () => {
      ...
      if (
        (o.routes.forEach((t) => {
          var r = t.service,
            o = t.route || '';
          if (o) {
            if (o.startsWith('/') || o.endsWith('/')) throw new Error(`路由格式错误，不能以'/'开头或结束。${JSON.stringify({ service: r, route: o })}`);
            ...
            t.routes.forEach((t) => {
              ...
              if (a) {
                if (a.startsWith('/') || a.endsWith('/'))
                  throw new Error(`路由格式错误，不能以'/'开头或结束。${JSON.stringify({ service: r, action: i, route: a })}`);
                if (a.includes('}{')) throw new Error(`路由格式错误，每个参数匹配符之间需要有其它字符。${JSON.stringify({ service: r, action: i, route: a })}`);
              }
              (s = o && a ? `${o}/${a}` : !o && a ? a : o && !a ? o : r),
                t.httpMethod
                  ? ('string' == typeof t.httpMethod && (t.httpMethod = [t.httpMethod]), (n = t.httpMethod))
                  : i.toUpperCase().startsWith('GET')
                  ? (n = ['GET'])
                  : i.toUpperCase().startsWith('POST') ||
                    i.toUpperCase().startsWith('CREATE') ||
                    i.toUpperCase().startsWith('ADD') ||
                    i.toUpperCase().startsWith('INSERT')
                  ? (n = ['POST'])
                  : i.toUpperCase().startsWith('PUT') || i.toUpperCase().startsWith('UPDATE')
                  ? (n = ['PUT'])
                  : i.toUpperCase().startsWith('DELETE') || i.toUpperCase().startsWith('REMOVE')
                  ? (n = ['DELETE'])
                  : i.toUpperCase().startsWith('PATCH') && (n = ['PATCH']),
                e.push({
                  routeTemplate: s,
                  httpMethod: n,
                  service: r,
      ...
      console.log(e),
        (e = e.sort((e, t) => {
          ...
          if (r.length === o.length) {
            var i =
                e.routeTemplate.replace(new RegExp('\\{[^\\}]+\\}', 'g'), EXPLAIN_ROUTE_PARAMETER_MATCHER).split(EXPLAIN_ROUTE_PARAMETER_MATCHER).length - 1,
              a = t.routeTemplate.replace(new RegExp('\\{[^\\}]+\\}', 'g'), EXPLAIN_ROUTE_PARAMETER_MATCHER).split(EXPLAIN_ROUTE_PARAMETER_MATCHER).length - 1;
            if (i > a) return !0;
          }
        })),
        console.log(e);
      var r = e[0],
        i = e.filter((e) => {
          if (
            r.routeTemplate.replace(new RegExp('\\{[^\\}]+\\}', 'g'), EXPLAIN_ROUTE_PARAMETER_MATCHER) ===
            e.routeTemplate.replace(new RegExp('\\{[^\\}]+\\}', 'g'), EXPLAIN_ROUTE_PARAMETER_MATCHER)
          )
            return e;
        });
      ...
      f.forEach((e, t) => {
          ...
          if (e === EXPLAIN_ROUTE_PARAMETER_MATCHER) E[o.replace(new RegExp('\\{|\\}', 'g'), '')] = r;
            ...
            for (var i = e.split(EXPLAIN_ROUTE_PARAMETER_MATCHER), a = r, s = o, n = 0; n < i.length - 1; n++) {
              ...
              c && ((l += `^${c}`), (f += `^${c}`)),
                (l += '{.+?}'),
                (f += '.+'),
                p && ((l += `${p}`), (f += `?${p}`), n === i.length - 2 && ((l += '$'), (f += '$')));
              var u = new RegExp(l, 'i').exec(s)[0].replace(new RegExp(`(^${c})\\{|\\}(${p}$)`, 'gi'), ''),
                R = new RegExp(f, 'i').exec(a)[0].replace(new RegExp(`(^${c})|(${p}$)`, 'gi'), '');
              (s = s.substring(s.indexOf(`${c}{${u}}`) + `${c}{${u}}`.length, s.length)),
                (a = a.substring(a.toLowerCase().indexOf(`${c}${R}`.toLowerCase()) + `${c}${R}`.length, a.length)),
                (E[u] = R);
            }
      }),
        (a = r.service),
        (s = r.action),
        (p = Object.assign({}, E, p)),
        (t.routeTemplate = r.routeTemplate);
    };
  if (n && '/' != n) u();
  else if (a) {
  ...
  (t.service = require(`${i.baseDir}${i.serviceDir}${a}`)),
    (t.isHttp = E),
    (t.httpMethod = c),
    (e[i.paramsKey] = p),
    (e[i.serviceKey] = a),
